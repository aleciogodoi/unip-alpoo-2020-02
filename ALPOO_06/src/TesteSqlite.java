...
public class TesteSqlite extends Tela implements ActionListener {
	...
	public Connection conectar (){
		try	{
			Connection c;                                      // Objeto de conexão
			Class.forName("org.sqlite.JDBC");                  // verificando o driver JDBC
			System.out.println("Driver JDBC encontrado!");     
			pathBD = System.getProperty("user.dir").replace("\\", "/"); // diretorio do projeto
			String bdUrl = "jdbc:sqlite:"+pathBD+"/bd/ALPOO.db"; // url do banco --> diretorio do projeto + "/bd/ALPOO.db"
			System.out.println(pathBD);
			System.out.println(bdUrl);
			
			c = DriverManager.getConnection(bdUrl);       // Cria conexão
	...
	public void executeComando (String txtSQL){
		try{
			stmt = conn.createStatement();           // Objeto para comando SQL
			System.out.println("Comando: " + txtSQL);

			if (txtSQL.toUpperCase().contains("SELECT")) {
				ResultSet rs = stmt.executeQuery (txtSQL);  // Objeto ResultSet (Onde estara o resultado do SELECT)
