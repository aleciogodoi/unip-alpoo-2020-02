...
public class Cadastro extends JFrame implements _________________ {
	...
	private static _________________ conn = null; 
	private static _________________ comandoSQL;
	...
	Cadastro(){
		...
		btSalvar._________________(this);
		btLimpar._________________(this);
		btPesquisar._________________(this);
		btSair._________________(this);
	...
	public boolean pesquisar(String texto, String campo, boolean atualizaTela){
		...
		conn = conexao._________________();
		try {
			if (campo=="IdUsuario") {
			...
			comandoSQL =  conn._________________();
			ResultSet rs = comandoSQL._________________(sql);
			
			if (rs.next()){
				if (atualizaTela) {
					txtCodigo.setText(String.valueOf(rs.getInt("IdUsuario")));
					txtNome.setText(rs._________________("nome"));
					txtEmail.setText(rs._________________("email"));
					txtTelefone.setText(rs._________________("telefone"));
	...
	public void salvar(){
		...
		try {
			...
			System.out.println(sql);
			comandoSQL =  conn._________________();
			comandoSQL._________________(sql);
