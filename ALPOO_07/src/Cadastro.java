...
public class Cadastro extends JFrame implements ActionListener {
	...
	JButton btSalvar, btPesquisar, btLimpar, btSair, btUsuarios, btExcluir;
	static Cadastro t01;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btSair) {
			...
			this.salvar();
		} else if (e.getSource() == btUsuarios) {
	...
	public void excluir() {
		if (txtCodigo.getText().isEmpty()) {
			JOptionPane.showMessageDialog(this, "Digite um código de usuário para exclusão!");
		} else {
			if(JOptionPane.showConfirmDialog(this, "Deseja Excluir?", "Excluir", JOptionPane.YES_NO_OPTION ) == JOptionPane.YES_OPTION) {
				String sql = "Delete From Usuario Where idUsuario=" +txtCodigo.getText();
				System.out.println(sql);
				conn = conexao.conectar();                       // Objeto de conexão com o banco de dados
				try {
					comandoSQL = conn.createStatement();         // objeto do para execução do comando SQL
					comandoSQL.executeUpdate(sql);               // Executa comandos SQL do tipo insert/update/delete
	...
	public void mostrarUsuarios(){
		String usuarios = "Todos os usuários\n\n";   // String para todos o usuários
		String sql = "Select * From Usuario";        // String com o comando Select
		conn = conexao.conectar();                   // Objeto com a conexão com o banco de dados
		try {
			comandoSQL = conn.createStatement();     // Objeto para execução do comando SQL
			ResultSet rs;                            // Objeto ResultSet - linhas e colunas com os dados do Select
			rs = comandoSQL.executeQuery(sql);	     // Executando "Select * From Usuario"
			while (rs.next()) {
				usuarios += "["+ rs.getInt("IDUsuario")+", ";
				usuarios += rs.getString("nome")+", ";
				usuarios += rs.getString("endereco")+", ";
				usuarios += rs.getString("email")+", ";
				usuarios += rs.getString("telefone")+"]\n";
			}
